...
def rows_to_list(rows, cursor=None):
...
def get_messages():
    ...
    result = rows_to_list(messages, cur)
    conn.close()
...
def get_all_analytics():
    ...
    result = rows_to_list(analytics_data, cur)
    conn.close()
...
def get_admin_stats():
    ...
    result = cur.fetchone()
    messages_count = result[0] if DB_TYPE == 'mysql' else result['count']
    
    cur.execute("SELECT COUNT(*) as count FROM ai_chats")
    result = cur.fetchone()
    chats_count = result[0] if DB_TYPE == 'mysql' else result['count']
    
    cur.execute("SELECT COUNT(*) as count FROM analytics")
    result = cur.fetchone()
    events_count = result[0] if DB_TYPE == 'mysql' else result['count']
